...
download_and_extract_backhaul() {
    ...
    case "$ARCH" in
        x86_64)
            DOWNLOAD_URL="https://github.com/Musixal/Backhaul/releases/download/v0.7.2/backhaul_linux_amd64.tar.gz"
            ;;
        arm64|aarch64)
            DOWNLOAD_URL="https://github.com/Musixal/Backhaul/releases/download/v0.7.2/backhaul_linux_arm64.tar.gz"
            ;;
        *)
...
edit_config_file(){
    local config_path="$1"
    echo
    read -p "Press any key to open the config file with 'nano'..."
    nano "$config_path"
    echo
    press_key
}
...
tunnel_management() {
    ...
    colorize green "3) Edit this tunnel's config file" # New option
	colorize reset "4) View service logs"
    colorize reset "5) View service status"
	echo 
    ...
    case $choice in
        ...
        3) edit_config_file "$selected_config" ;; # Call the new function
